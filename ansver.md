...
### Сравнение VLA и alloca ?? проверить__________________и спросить почему нужно проверить у КИРИЛЛА__
...
___Оформление функций, которые входят в состав динамической библиотеки___
- Для Linux: не нужно
- Для Windows: 
    + в библиотеке: __declspec(dllexport) void __cdecl f(...);
    + в приложении: __declspec(dllimport) void __cdecl f(...);

___Использование dllimport/dllexport___    
В Windows   
...
___API Linux для работы с динамическими библиотеками___   
```c
dlfcn.h
```
...
___API Windows для работы с динамическими библиотеками___
```c
windows.h
```
