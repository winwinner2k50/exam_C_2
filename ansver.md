...
## Рассказать про 2 представления, сравнить между собой.
...
<img src="image/dynamic_matrix1.png" alt="dynamic_matrix1" width="50%" height="50%">
...
для обращение к элементу `i j` введём переменную `k`, `k = i * m + j`
...
**Недостатки:**
- Отладчик использования памяти (например, valgrind) не
может отследить выход за пределы строки.
...
<img src="image/dynamic_matrix2.png" alt="dynamic_matrix2" width="50%" height="50%">
...
Переставновка строк через обмен указателей|но в этом возможна ошибка
...
2. `char *(*(**foo[][8])())[];`
    ...
    - `*(**foo[][8])()` - матрица указателей на функцию которая принимает ничего
    - функция возращает массив указтелей char то есть с сигнатурой `char *(*return_array())[]`
...
если память выделеть не удалось возращает `NULL`
...
`n` - размер выделенной памяти (ПРИ ЭТОТ КОЛ-ВО ВЫДЕЛЕННОЙ ПАМЯТИ МОЖЕТ НЕ СООТВЕТСТВОВАТЬ КОЛИЧЕСТВУ СИМВОЛОВ)
...
```c
ssize_t qg_getline(char **lineptr, size_t *n, FILE *stream) {
    ...
    while ((c = fgetc(stream)) != EOF) {
        if (pos + 1 >= *n) {
            *n *= 2; // Увеличиваем буфер в 2 раза
    ...
    return pos; // Возвращаем количество символов
}
```
...
Не не не нихуя я это реализовывать не буду там вот делали snprintf в лабе было вот типо из него чё то там бибим бим бам бам и в буфер обмнена нахуй
...
Из за **«поверхностного копирования»** могут происходить ошибки использовать его можно только если у нас нас динамических полей в структуре (ну или же мы дейтвительно хотим ссылать всегда к одной области памяти), стоит понимать что при пересвоинии мы теряем указатель доступ памяти к которой он указывал.
